...
| 2026-05-13 | **Engineering Infrastructure — Widgetbook entry alias + Flutter loader upgrade + deploy.sh + web_deploy.yml** — Created `lib/main_widgetbook.dart` standard entry point (`fvm flutter run --target lib/main_widgetbook.dart`). Upgraded `web/index.html` Flutter init from deprecated `loadEntrypoint()` to modern `_flutter.loader.load()` with auto SW version injection. Created `deploy.sh` as single-source-of-truth web deployment pipeline (clean → build → rm canvaskit/NOTICES/widgetbook → inject part files → gzip → size audit). Unified `web_deploy.yml` CI/CD: `--wasm` → `--web-renderer html`, added post-build cleanup + build size audit in job summary. | ✅ |
