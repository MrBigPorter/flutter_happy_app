...
# 📜 Lucky IM Project **Grand Master Log** (v4.0 - v5.3.3)

> **🕒 终极封档**: 2026-02-09 21:30 (PST)
> **🚀 当前版本**: **v5.3.3 (The Visual Revolution & Chronos)**
> **🌟 总体进度**: **全栈基建与极致体验完全封顶**。
> 本日志是 Lucky IM 的技术编年史，完整记录了从单机 MVP 到工业级分布式 IM 的演进历程，涵盖了一致性、高可靠性、极致媒体体验、后端架构及基础性能优化的所有关键节点。
...
## 🛡️ 架构铁律 (The Iron Rules - v5.3.3)

*这是项目的最高准则，任何代码提交不得违反。*

1. **路径相对化原则**: 数据库持久化**严禁存储绝对路径**，必须通过 `AssetManager` 在运行时动态还原。
2. **数据防御原则**: `merge` 操作必须**优先保留本地**的 `localPath` (高清) 和 `previewBytes` (内存快照)，严禁被服务端空值覆盖。
3. **单向数据流**: UI **只读 DB**，Pipeline/Repo **负责写 DB**。禁止 UI 直接渲染 API 返回的数据。
4. **服务端权威原则**: 当服务端返回 `unread=0` 时，本地必须无条件强制清零 (`forceClearUnread`)。
5. **指纹对齐原则**: 跨页面复用媒体缓存，`ImageProvider` 的 URL 和 Headers 必须字符级匹配（锁死 DPR=3.0）。
6. **视觉兜底原则** **[NEW]**: 图片加载必须使用 `Stack`，底层必须垫着 `previewBytes`，严禁让用户看到白屏或 Loading。
7. **时间统一原则** **[NEW]**: 所有的逻辑判断（排序、超时）必须使用 `ServerTimeHelper.now()`。
8. **资源分级原则**: 视频播放必须遵循 **Local -> Cache -> Streaming** 的降级策略。
9. **Web 环境隔离**: 非 `kIsWeb` 保护下，**严禁调用 `dart:io**`。
