...
> **最新战绩**：
> 1. **智能直传**：实现了基于体积 () 与分辨率 () 的智能过滤逻辑，小视频秒发不损画质。
> 2. **双引擎补丁**：采用 `FFprobe` + `Native Retriever` (via VideoCompress) 双重校验，彻底解决了视频时长/宽高抓取不到（）的顽疾。
> 3. **码率硬化**：FFmpeg 指令锁死 、，引入 `faststart` 标志位，确保 CDN 视频流在 Web/App 端秒开。
> 4. **黑屏防御**：封面提取点强制固定为视频第  秒，完美避开第  帧可能存在的黑屏或无效画面。
...
9. ** [NEW] 智能旁路原则**: 满足“体积小、分辨率低”条件的媒体必须跳过压缩，保护原始画质并节省 CPU。
...
* [✓] **[P0] 自动缩放约束** (强制约束长边 720p，防止超大视频撑爆带宽)。
* [✓] **[P0] 智能直传过滤** (小文件跳过 FFmpeg，减少用户等待感)。
